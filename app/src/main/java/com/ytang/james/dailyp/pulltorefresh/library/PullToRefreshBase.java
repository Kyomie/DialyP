...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
    ...
    private static final int MIN_REFRESH_ANIMATION_TIME = 1000;
    ...
    private AbsPullToRefreshAnimationLayout mHeaderLayout;
    private AbsPullToRefreshAnimationLayout mFooterLayout;
    ...
    private boolean isDelayedFlag = false;
    ...
    public final void startPullDownToRefresh() {
        mCurrentMode = Mode.PULL_DOWN_TO_REFRESH;
        setRefreshing();
        mOnRefreshListener2.onPullDownToRefresh(this);
    }

    public final void startPullUpToRefresh() {
    ...
    public void setLastUpdatedLabel(CharSequence label) {
        setLastUpdatedLabel(label, true);
    }

    public void setLastUpdatedLabel(CharSequence label, boolean forceLayout) {
    ...
    protected void onRefreshing(final boolean doScroll) {
        if (mMode.canPullDown() || mMode.canPullUp()) {
    ...
    /**
     * Called when delayed animation time
     * or when after {@link #onRefreshComplete()}
     */
    private void performRefreshComplete() {
        if (isDelayedFlag && isCompleteFlag) {
            isDelayedFlag = false;
    ...
    protected void onReset() {
        ...
        if (mMode.canPullDown()) {
            mHeaderLayout.reset(false);
        }
        if (mMode.canPullUp()) {
            mFooterLayout.reset(false);
        }
    ...
    protected AbsPullToRefreshAnimationLayout createLoadingLayout(Context context, Mode mode,
                                                                  TypedArray attrs) {
    ...
    protected final AbsPullToRefreshAnimationLayout getFooterLayout() {
    ...
    protected final AbsPullToRefreshAnimationLayout getHeaderLayout() {
     ...
     *             method then use {@link #setOnPullEventListener(OnPullEventListener)
    ...
    protected void updateUIForMode() {
        ...
        if (mMode.canPullDown()) {
            addViewInternal(mHeaderLayout, 0, new LinearLayout.LayoutParams(
        ...
        if (mMode.canPullUp()) {
            addViewInternal(mFooterLayout, new LinearLayout.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT));
        }
    ...
    private void pullEvent() {
        ...
        switch (mCurrentMode) {
            case PULL_UP_TO_REFRESH:
                newScrollY = Math.round(Math.max(mInitialMotionY - mLastMotionY, 0)
                        / FRICTION);
                itemHeight = mFooterHeight;
                break;
            case PULL_DOWN_TO_REFRESH:
            default:
                newScrollY = Math.round(Math.min(mInitialMotionY - mLastMotionY, 0)
                        / FRICTION);
        ...
        if (newScrollY != 0) {
            ...
            switch (mCurrentMode) {
                case PULL_UP_TO_REFRESH:
                    mFooterLayout.pullY(scale);
                    break;
                case PULL_DOWN_TO_REFRESH:
                    mHeaderLayout.pullY(scale);
                    break;
                default:
                    break;
            }
            ...
            if (mState != State.PULL_TO_REFRESH
                    && itemHeight >= Math.abs(newScrollY)) {
    ...
    public void setStartRefresh() {
        mLastMotionY = mHeaderHeight * 2 + 3;
        mInitialMotionY = 0;
        pullEvent();
    }
    ...
    public static enum AnimationStyle {
        ...
        public AbsPullToRefreshAnimationLayout createLoadingLayout(Context context, Mode mode,
                                                                   TypedArray attrs) {
            switch (this) {
                case PROPERTY:
                default:
     ...
     * (such as sounds). See ( {@link PullToRefreshAdapterViewBase#setOnPullEventListener}.
     */
    public static interface OnPullEventListener<V extends View> {
         ...
         * @param direction - One of {@link Mode#PULL_UP_TO_REFRESH} or {@link Mode#PULL_UP_TO_REFRESH}
         *          depending on which
